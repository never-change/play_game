...
const jsonrpc = (url, params) => {
  ...
  return _fetch(url, options, 100000)
    .then(res => {
      //  console.log('1st',res)
...
class RPC {
  ...
  async login(params) {
    ...
    const data = await jsonrpc(url, { login, password, db: this.db, role, type: 'account' })
    const { code } = data
    if (!code) {
  ...
  async register(params) {
    const { db, login, password, role, phone, email } = params
    const url = `${this.host}/json/user/register`
    if (db) {
      this.db = db
    }
    const data = await jsonrpc(url, { login, password, db: this.db, role, phone, email })
    return data
  }
